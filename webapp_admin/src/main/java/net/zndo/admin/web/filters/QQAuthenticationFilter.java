...
public class QQAuthenticationFilter extends AbstractAuthenticationProcessingFilter{
	...
	private final static String PARAM_NAME_CODE = "code";
    ...
    private final static String redirectUri = "http://www.test.net/admin/signin/qq";
	 ...
	 * @throws IOException
	 */
    private QQToken getQQToken(String tokenAccessApi) throws IOException{
    ...
    private String getOpenId(String accessToken) throws IOException{
        ...
        String openIdResult = document.text();
        Matcher matcher = Pattern.compile("\"openid\":\"(.*?)\"").matcher(openIdResult);
        if (matcher.find()){
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        QQToken qqToken = this.getQQToken(tokenAccessApi);
        if (qqToken != null){
	...
	/**
	 * QQToken 类 接收 QQ OAuth2 验证的返回数据
	 * 
	 * @author zndo
	 *
	 */
